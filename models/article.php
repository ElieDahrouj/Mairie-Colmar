...
function getArticle($getOneArticle){
    $db = dbConnect();
    $query = $db->prepare('SELECT * FROM article WHERE published_at <= NOW() AND is_published = 1 AND id = ?');
...
function getSecondaryPicture($getPictures){
    $db = dbConnect();
    $query = $db->prepare('SELECT * FROM image WHERE article_id = ? ORDER BY id ASC');
